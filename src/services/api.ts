import axios, { AxiosResponse } from 'axios';
import md5 from 'js-md5';

interface PropsImp {
  service: 'characters' | 'comics' | 'stories' | 'series';
}

const requestResolver = async ({
  service = 'characters',
}: PropsImp): Promise<AxiosResponse> => {
  const PUBLIC_KEY = 'be40780c1854261ee900fa2c17b576a9';
  const PRIVATE_KEY = 'cc97a9b583ff905769ae44e03bb61a23022a5d13';

  const timestamp = Number(new Date());
  const hash = md5.create();
  hash.update(timestamp + PRIVATE_KEY + PUBLIC_KEY);

  const response = await axios(
    `https://gateway.marvel.com/v1/public/${service}?limit=10&ts=${timestamp}&apikey=${PUBLIC_KEY}&hash=${hash}`,
  );

  return response.data;
};

export default requestResolver;
